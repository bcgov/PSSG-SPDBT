...
public class GDSDTeamLicenceAppBaseValidator<T> : AbstractValidator<T> where T : GDSDTeamLicenceAppBase
{
    public GDSDTeamLicenceAppBaseValidator()
    {
        ...
        RuleFor(r => r.DogInfoNewAccreditedSchool).NotEmpty().When(r => r.DogTrainedByAccreditedSchool);
        RuleFor(r => r.DogInfoNewAccreditedSchool.DogName).NotEmpty().When(r => r.DogTrainedByAccreditedSchool);
