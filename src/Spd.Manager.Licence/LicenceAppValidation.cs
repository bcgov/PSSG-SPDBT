...
public class PersonalLicenceAppBaseValidator<T> : AbstractValidator<T> where T : PersonalLicenceAppBase
{
    public PersonalLicenceAppBaseValidator()
    {
        ...
        RuleFor(r => r.IsCanadianCitizen).NotEmpty().When(r => r.ApplicationTypeCode == ApplicationTypeCode.New || r.ApplicationTypeCode == ApplicationTypeCode.Renewal); //spdbt-4110
        //residential address
