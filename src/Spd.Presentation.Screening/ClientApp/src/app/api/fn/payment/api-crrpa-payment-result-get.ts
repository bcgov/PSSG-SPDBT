...
export interface ApiCrrpaPaymentResultGet$Params {
  trnApproved?: number;
  messageText?: string;
  cardType?: string;
  trnOrderId?: string;
...
export function apiCrrpaPaymentResultGet(http: HttpClient, rootUrl: string, params?: ApiCrrpaPaymentResultGet$Params, context?: HttpContext): Observable<StrictHttpResponse<ActionResult>> {
  const rb = new RequestBuilder(rootUrl, apiCrrpaPaymentResultGet.PATH, 'get');
  if (params) {
...
apiCrrpaPaymentResultGet.PATH = '/api/crrpa/payment-result';
