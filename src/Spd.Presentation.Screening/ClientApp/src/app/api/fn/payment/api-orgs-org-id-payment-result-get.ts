...
export interface ApiOrgsOrgIdPaymentResultGet$Params {
  trnApproved?: number;
  messageText?: string;
  cardType?: string;
  trnOrderId?: string;
...
export function apiOrgsOrgIdPaymentResultGet(http: HttpClient, rootUrl: string, params: ApiOrgsOrgIdPaymentResultGet$Params, context?: HttpContext): Observable<StrictHttpResponse<ActionResult>> {
  const rb = new RequestBuilder(rootUrl, apiOrgsOrgIdPaymentResultGet.PATH, 'get');
  if (params) {
...
apiOrgsOrgIdPaymentResultGet.PATH = '/api/orgs/{orgId}/payment-result';
