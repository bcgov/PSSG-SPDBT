...
export class PssoaComponent implements OnInit {
	...
	async ngOnInit(): Promise<void> {
		...
		const orgData = (this.location.getState() as any).pssoaOrgData;
		if (orgData) {
	...
	async onRegisterWithBcServicesCard(): Promise<void> {
		...
		this.utilService.setSessionData(this.utilService.PSSOA_PORTAL_STATE_KEY, stateInfo);

		const nextUrl = await this.authProcessService.initializePssoa();
		if (nextUrl) {
