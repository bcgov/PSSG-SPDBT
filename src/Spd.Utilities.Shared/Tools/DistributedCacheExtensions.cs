//source: https://github.com/mgravell/DistributedCacheDemo/blob/main/DistributedCacheExtensions.cs
...
public static class DistributedCacheExtensions
{
    ...
    public static Task SetAsync<T>(this IDistributedCache cache, string key, T value,
        TimeSpan absoluteExpirationRelativeToNow, CancellationToken cancellation = default)
        => SetAsync<T>(cache, key, value, new DistributedCacheEntryOptions { AbsoluteExpirationRelativeToNow = absoluteExpirationRelativeToNow }, cancellation);
        ...
        DistributedCacheEntryOptions? options = null, CancellationToken cancellation = default)
    {
        ...
        {   // not recommended; cache expiration should be considered
            // important, usually
            return cache.SetAsync(key, bytes, cancellation);
        }
        else
        {
            return cache.SetAsync(key, bytes, options, cancellation);
        }
    }

    public static ValueTask<T?> GetAsync<T>(this IDistributedCache cache, string key, CancellationToken cancellation = default)
        => GetAsyncShared<int, T>(cache, key, state: 0, null, new DistributedCacheEntryOptions { }, cancellation);
    ...
    public static ValueTask<T?> GetAsync<T>(this IDistributedCache cache, string key, Func<CancellationToken, ValueTask<T>> getMethod,
        TimeSpan absoluteExpirationRelativeToNow, CancellationToken cancellation = default)
        => GetAsync<T>(cache, key, getMethod, new DistributedCacheEntryOptions { AbsoluteExpirationRelativeToNow = absoluteExpirationRelativeToNow }, cancellation);
        ...
        DistributedCacheEntryOptions? options, CancellationToken cancellation)
    {
            ...
            CancellationToken cancellation)
        {
            ...
            var result = getMethod switch
            {
                ...
                Func<T> get => get(),
                // a special use-case for get only
                null => default,
                _ => throw new ArgumentException("Failed to invoke getMethod", nameof(getMethod)),
            };
            if (result is not null)
            {
                // only cache non-null result
                await cache.SetAsync(key, result, options, cancellation);
            }
            return result;
        }
    }
