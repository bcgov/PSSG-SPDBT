...
@Component({
	selector: 'app-step-police-background-renew-and-update',
	template: `
		<section class="step-section">
			<div class="step">
				<app-renewal-alert [applicationTypeCode]="applicationTypeCode"></app-renewal-alert>

				<app-renewal-value-changed
					[originalValue]="original_isPoliceOrPeaceOfficer"
					yesString="I am a Police Officer or Peace Officer"
					noString="I am a NOT Police Officer or Peace Officer"
					(valueChanged)="onValueHasChanged($event)"
				></app-renewal-value-changed>

				<ng-container *ngIf="original_isPoliceOrPeaceOfficer_hasChanged">
...
export class StepPoliceBackgroundRenewAndUpdateComponent implements OnInit, LicenceChildStepperStepComponent {
	booleanTypeCodes = BooleanTypeCode;
	policeOfficerRoleCodes = PoliceOfficerRoleCode;
	policeOfficerRoleTypes = PoliceOfficerRoleTypes;
	applicationTypeCodes = ApplicationTypeCode;

	original_isPoliceOrPeaceOfficer: any = null;
	original_isPoliceOrPeaceOfficer_hasChanged = false;
	...
	ngOnInit(): void {
		...
		this.original_isPoliceOrPeaceOfficer = this.isPoliceOrPeaceOfficer.value === BooleanTypeCode.Yes ? true : false;
	}

	onValueHasChanged(value: boolean) {
		this.original_isPoliceOrPeaceOfficer_hasChanged = value;
	}
	...
	isFormValid(): boolean {
		if (!this.original_isPoliceOrPeaceOfficer_hasChanged) {
			return true;
		}
