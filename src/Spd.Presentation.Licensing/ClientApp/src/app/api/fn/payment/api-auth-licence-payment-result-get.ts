...
export interface ApiAuthLicencePaymentResultGet$Params {
  trnApproved?: number;
  messageText?: string;
  cardType?: string;
  trnOrderId?: string;
...
export function apiAuthLicencePaymentResultGet(http: HttpClient, rootUrl: string, params?: ApiAuthLicencePaymentResultGet$Params, context?: HttpContext): Observable<StrictHttpResponse<ActionResult>> {
  const rb = new RequestBuilder(rootUrl, apiAuthLicencePaymentResultGet.PATH, 'get');
  if (params) {
...
apiAuthLicencePaymentResultGet.PATH = '/api/auth-licence/payment-result';
