...
export interface ApiBusinessBizIdPaymentResultGet$Params {
  trnApproved?: number;
  messageText?: string;
  cardType?: string;
  trnOrderId?: string;
...
export function apiBusinessBizIdPaymentResultGet(http: HttpClient, rootUrl: string, params: ApiBusinessBizIdPaymentResultGet$Params, context?: HttpContext): Observable<StrictHttpResponse<ActionResult>> {
  const rb = new RequestBuilder(rootUrl, apiBusinessBizIdPaymentResultGet.PATH, 'get');
  if (params) {
...
apiBusinessBizIdPaymentResultGet.PATH = '/api/business/{bizId}/payment-result';
